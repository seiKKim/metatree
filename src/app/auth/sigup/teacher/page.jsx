...
const TeacherInfo = () => {
  ...
  const [roleState, setRoleState] = useState("선생님");
  ...
  const [isTeacherNumberAvailable, setIsTeacherNumberAvailable] = useState(null);
  const [teacherNumberWarning, setTeacherNumberWarning] = useState("");
  ...
  //교원번호 중복 체크
  const handleTeacherNumberChange = async (e) => {
    const newTeacherNumber = e.target.value;
    setTeachernumber(newTeacherNumber);
  
    if (newTeacherNumber.length === 0) {
      setTeacherNumberWarning("");
      setIsTeacherNumberAvailable(null);
      return;
    }
  
    try {
      const response = await fetch(`/api/member/checkTeacherNumber?teacherNumber=${newTeacherNumber}`);
      const data = await response.json();
  
      if (response.ok) {
        setIsTeacherNumberAvailable(true);
        setTeacherNumberWarning("사용 가능한 교원번호입니다.");
      } else {
        setIsTeacherNumberAvailable(false);
        setTeacherNumberWarning(data.error || "이미 사용 중인 교원번호입니다.");
      }
    } catch (error) {
      console.error("교원번호 중복 체크 오류:", error);
      setTeacherNumberWarning("교원번호 중복 체크에 실패했습니다.");
      setIsTeacherNumberAvailable(null);
    }
  };
            ...
            <div className={styles.inputWrap}>
              ...
              <div className={styles.inputBox}>
                <input
                  ...
                  placeholder="교원번호 입력"
                  value={teachernumber}
                  onChange={handleTeacherNumberChange} // 여기에 새로운 핸들러 추가
                />
              </div>
            </div>
            {teacherNumberWarning && ( // 경고 메시지 표시
              <div className={styles.warning}>
                <span style={{ color: "red", fontWeight: "bold" }}>
                  {teacherNumberWarning}
                </span>
              </div>
            )}
            <div className={styles.inputWrap}>
